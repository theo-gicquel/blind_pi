...
#include "rgb_lcd.h"
 ...
 // initialisation de l'afficheur RGB_LCD
rgb_lcd lcd;
...
// Setup -> éxecuté une seule fois
void setup() { 

  int delai = 2000;
  pinMode(3, OUTPUT); // Relais
  pinMode(2, OUTPUT); // Relais
  pinMode(A1, INPUT); // Broche Tempint
  pinMode(A2, INPUT); // Broche Tempext
  pinMode(A4, INPUT); // Broche Batterie
  pinMode(A3, INPUT); // Broche Luminosité
...
/*~~~~~~~~~~~~~~~~ TEMPINT ~~~~~~~~~~~~~~~~ */
	/*afficher [Temp Int :_°C ] 
			   [              ]
	*/
  //lcd.setCursor(col, row)


  delay(delai);  // Attendre 5sec
  // Renre vierge l'écran
  lcd.clear();
  lcd.createChar(0, deg);
  // premier chiffre, colones, second chiffre ligne.
  lcd.setCursor(0, 0);
